            ...
            async with httpx.AsyncClient(timeout=None) as client:
                async with client.stream(
                    ...
                    if response.status_code != 200:
                        ...
                        logger.error(
                            f"[BackendClient] 分析 API 返回错误: {response.status_code} - {error_text}"
                        )
                        yield {
                            "agent_name": "System",
                    ...
                    async for line in response.aiter_lines():
                        ...
                        if line.startswith("data: "):
                            try:
                                ...
                                logger.debug(
                                    f"[BackendClient] SSE 事件: {data.get('agent_name')} - {data.get('status')}"
                                )
                                yield data
                            except json.JSONDecodeError as e:
                                logger.warning(
                                    f"[BackendClient] SSE JSON 解析失败: {e}, line={line}"
                                )
                                continue
    ...
    async def get_workflow_status(self) -> Dict[str, Any]:
        ...
        try:
            async with httpx.AsyncClient(timeout=30) as client:
                ...
                if response.status_code != 200:
                    logger.error(
                        f"[BackendClient] 获取工作流状态失败: {response.status_code}"
                    )
                    return {
                        "success": False,
                        "running": False,
                ...
                data = response.json()
                logger.debug(
                    f"[BackendClient] 工作流状态: running={data.get('running')}, step={data.get('current_step')}"
                )
