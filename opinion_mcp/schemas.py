...
class JobInfo(BaseModel):
    ...
    current_source: Optional[str] = Field(default=None, description="当前检索来源")
...
class WebhookData(BaseModel):
    ...
    source: Optional[str] = Field(default=None, description="来源代码")
    source_name: Optional[str] = Field(default=None, description="来源名称")
    source_count: Optional[int] = Field(default=None, description="来源数据量")
...
class AnalyzeTopicResponse(BaseModel):
    ...
    estimated_time_minutes: int = Field(default=10, description="预估时间（分钟）")
    sources: List[str] = Field(default_factory=list, description="检索来源")
    depth: str = Field(default="standard", description="分析深度")
...
class AnalysisResultResponse(BaseModel):
    ...
    sources_analyzed: List[str] = Field(default_factory=list, description="已分析来源")
    skipped_sources: List[str] = Field(default_factory=list, description="跳过的来源")
    evidence_count: int = Field(default=0, description="证据条目总数")
    source_stats: Dict[str, int] = Field(default_factory=dict, description="来源统计")
