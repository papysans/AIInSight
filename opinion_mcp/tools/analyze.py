...
async def analyze_topic(
    ...
    Args:
        ...
        source_names: 指定来源列表，如 hn, reddit, aibase 等。与 source_groups 互斥
...
async def get_analysis_result(job_id: Optional[str] = None) -> Dict[str, Any]:
    ...
    Returns:
        ...
        - cards: Dict - 数据卡片 URL (title_card, debate_timeline, trend_analysis, platform_radar)
        - ai_images: List[str] - AI 生成图片 URL 列表
        - summary: str - 核心观点摘要
        - insight: str - 深度洞察分析
        - sources_analyzed: List[str] - 已分析来源列表
        - skipped_sources: List[str] - 跳过的来源列表
        - evidence_count: int - 证据条目总数
        - source_stats: Dict - 各来源数据量统计
    ...
    return {
        ...
        "cards": result.cards.model_dump() if result.cards else None,
...
def _map_agent_to_step(agent_name: str) -> Optional[str]:
    ...
    agent_step_map = {
        ...
        "crawler_agent": "source_retriever",
        "crawler": "source_retriever",
        "reporter": "reporter",
        "analyst": "analyst",
        "debater": "debater",
        "writer": "writer",
...
async def generate_topic_cards(
    ...
    """
    logger.info(f"[generate_topic_cards] job_id={job_id}, card_types={card_types}")
    ...
    try:
        async with httpx.AsyncClient(timeout=60) as client:
            resp = await client.post(
                f"{config.BACKEND_URL.rstrip('/')}/api/topic/cards",
                json={
        ...
        # Store card URLs on the job result
        card_urls = {}
        ...
        logger.error(f"[generate_topic_cards] 失败: {e}")
        return {
            "success": False,
