...
- `{today_date}` — 搜索 query 中出现时，替换为运行时的当天日期（如 `March 30 2026`、`2026-03-30`）。**禁止硬编码具体日期。**
...
> **时效性强制要求**：
> - 所有搜索**必须**使用 `freshness: "24h"` 参数（如搜索工具支持）
> - 如搜索工具不支持 freshness 参数，则在 query 中嵌入当天日期 `{today_date}`
> - 两者都支持时，**同时使用**（参数 + query 日期双保险）

| 轮次 | Query | freshness | 说明 |
|------|-------|-----------|------|
| 1 | `"AI news {today_date}" site:aibase.com` | `24h` | AIbase 今日新闻 |
| 2 | `"今日AI {today_date}" site:jiqizhixin.com OR site:qbitai.com` | `24h` | 机器之心 / 量子位 |
| 3 | `"AI" site:techcrunch.com {today_date}` | `24h` | TechCrunch 最新 |
| 4 | `"AI breakthrough" OR "AI release" {today_date} {current_year}` | `24h` | 突破性进展 |
| 5 | `"AI" site:github.com trending` | `week` | GitHub 热门 AI 项目（趋势类允许放宽到一周） |
...
示例：
```
[AIBase] GPT-5 发布 - OpenAI 发布 GPT-5，推理能力大幅提升 (2026-03-30)
[量子位] 国产大模型新突破 - 某厂商发布百亿参数开源模型，性能超越同量级 SOTA (2026-03-30)
[TechCrunch] Google DeepMind new paper - 提出新架构，训练效率提升 3× (2026-03-30)
```

### 时效性硬过滤（Phase 1 出口检查）

摘要提取完毕后，**立即执行日期过滤**，在进入 Phase 2 之前丢弃过期条目：

1. 每条结果必须标注发布日期（从搜索结果中提取，无法确定日期的标注 `unknown`）
2. **硬规则**：丢弃发布日期距今 **超过 48 小时** 的条目（保留今天 + 昨天）
3. 日期标注为 `unknown` 的条目保留，但在 Phase 2 评分时 Timeliness 维度得分上限为 5
4. 过滤后如果剩余条目 < 10 条，不补搜 — 宁少勿旧

```
⚠️ 时效性过滤示例：
今天是 2026-03-30

✅ 保留：(2026-03-30)、(2026-03-29)、(unknown)
❌ 丢弃：(2026-03-28)、(2026-03-26)、(2026-03-25)
```
...
total = (novelty × 0.2 + impact × 0.3 + credibility × 0.2 + timeliness × 0.3)，保留一位小数。
...
1. `daily_topics.json` — `daily_topics[]` 数组（榜单排名数据）
2. `search_snapshot.json` — Phase 1 Pulse 的原始搜索结果快照，格式：
   ```json
   [
     {
       "title": "文章标题",
...
搜索快照来源优先级：
1. 当前会话内存中的 Phase 1 原始搜索结果
2. 若内存不可用，从 `output/daily/{YYYY-MM-DD}/search_snapshot.json` 恢复，按 `related_topics` 过滤出与选中话题相关的条目
