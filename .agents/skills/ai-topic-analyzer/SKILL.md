...
- 如果发布前发现小红书未登录，或发布结果返回 `login_required=true`，必须调用 `get_xhs_login_qrcode`
- 拿到二维码后，优先把 `qr_image_url` 告诉用户；如果没有公网基址，就展示 `qr_image_route` 和 `qr_image_path`
- 给出二维码地址后，不要自动重试发布；等待用户回复“已扫码”或再次确认
...
如果用户说“给我小红书登录二维码”“发个扫码地址”“登录下小红书”，直接调用：

```json
{}
```

对应工具：`get_xhs_login_qrcode`
