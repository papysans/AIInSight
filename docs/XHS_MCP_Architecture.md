...
│ FastAPI (api)                   │
│  /api/xhs/status                │  ←  检查登录
│  /api/xhs/login-qrcode           │  ←  获取二维码
│  /api/xhs/login/reset            │  ←  重置登录
...
- `docker-compose.yml` — 当前 main 分支主入口，默认包含 `api` / `mcp` / `renderer` / `xhs-mcp`
- `docker-compose.xhs.yml` — 历史/叠加配置文件，保留给显式 overlay 场景；当前日常启动优先使用 `docker-compose.yml`
...
如果 `api` 因 `8000` 端口冲突无法启动，通常是旧 worktree 项目的容器仍在占用端口；应先停止旧的 `*-api-1` / `*-mcp-1` / `*-renderer-1` 容器。
...
另外，若 `mcp` 容器日志出现 `ImportError: cannot import name 'reset_xhs_login' from 'opinion_mcp.tools'`，说明镜像没有包含最新的工具导出修复；应重新构建 `api` / `mcp` 镜像并重启四容器栈。
