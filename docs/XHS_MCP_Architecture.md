...
| 端点 | 说明 |
|------|------|
| `GET /api/xhs/status` | 检查 sidecar 可用性和登录状态 |
| `GET /api/xhs/login-qrcode` | 获取登录二维码 |
| `GET /api/xhs/check-login-session/{session_id}` | 轮询扫码状态 |
| `POST /api/xhs/submit-verification` | 提交短信验证码 |
| `POST /api/xhs/login/reset` | 重置登录状态 |
...
│  /api/xhs/check-login-session/  │
│  /api/xhs/submit-verification   │
│  /api/xhs/login/reset           │
│  /api/xhs/publish               │
└───────┬───────────┬─────────────┘
        │           │
...
`XiaohongshuPublisher` (app/services/xiaohongshu_publisher.py) 作为适配层：
...
MCP Gateway (account_id from API key)
  → opinion_mcp/tools/publish.py (account_id param)
    → opinion_mcp/services/backend_client.py (account_id as query param)
      → app/api/endpoints.py (account_id query param)
        → XiaohongshuPublisher (account_id → ShunL MCP "account" field)
```
