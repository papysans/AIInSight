...
from app.services.user_settings import (
    get_effective_llm_keys,
    get_agent_llm_overrides,
...
class KeyManager:
    def __init__(self, keys: Sequence[str], provider_name: str = "Unknown"):
...
def _get_manager(provider_key: str, keys: List[str], provider_name: str) -> KeyManager:
    ...
    """
    account_id = (get_account_id() or "_default").strip() or "_default"
    pk = f"{account_id}:{(provider_key or '').strip().lower()}"
...
def get_llm(provider: str, model_name: str, api_key: Optional[str] = None):
    ...
    if provider == "google" or provider == "gemini":
        if api_key:
            ...
            keys = get_effective_llm_keys(
                provider_key="gemini", env_keys=settings.GEMINI_API_KEYS
            )
        ...
        if api_key:
            ...
            keys = get_effective_llm_keys(
                provider_key="kimi", env_keys=settings.MOONSHOT_API_KEYS
            )
        ...
        if api_key:
            ...
            keys = get_effective_llm_keys(
                provider_key="openai", env_keys=[settings.OPENAI_API_KEY]
            )
            current_key = keys[0] if keys else settings.OPENAI_API_KEY
            print(f"🔑 Using OpenAI Key: ...{current_key[-6:]}")
        return ChatOpenAI(
            model=model_name, temperature=0.7, api_key=cast(Any, current_key)
        )
        ...
        if api_key:
            ...
            keys = get_effective_llm_keys(
                provider_key="deepseek", env_keys=settings.DEEPSEEK_API_KEYS
            )
        ...
        if api_key:
            ...
            keys = get_effective_llm_keys(
                provider_key="doubao", env_keys=settings.DOUBAO_API_KEYS
            )
        ...
        if api_key:
            ...
            keys = get_effective_llm_keys(
                provider_key="zhipu", env_keys=settings.ZHIPU_API_KEYS
            )
...
class ResilientChatModel:
    ...
    def __init__(self, configs, agent_name: Optional[str] = None):
        self.configs: List[Dict[str, Any]] = (
            configs if isinstance(configs, list) else [configs]
        )
    ...
    async def ainvoke(self, messages, **kwargs):
        ...
        for idx, config in enumerate(self.configs):
            ...
            if (
                agent_config
                and str(agent_config["provider"]).lower()
                == str(_PROVIDERKEY_TO_PROVIDER.get(provider.lower(), provider)).lower()
            ):
                api_key = agent_config["key"]
                if agent_config["model"]:
                    model = agent_config["model"]
                print(
                    f"🎯 [Agent Override] Using {self.agent_name}'s API: {provider} ({model})"
                )
        ...
        raise Exception(
            f"❌ All {len(self.configs)} LLM providers failed:\n" + "\n".join(errors)
        )
...
def _apply_agent_override(
    agent_name: str, configs: Sequence[Dict[str, Any]]
) -> List[Dict[str, Any]]:
    """
    ...
    if agent_config:
        ...
        print(
            f"🎯 [Agent Override] {agent_name} -> {backend_provider} ({model or 'default'})"
        )
        ...
        return [
            {
                ...
                or configs[0].get(
                    "model", ""
                ),  # Use agent's model or fallback to default
            }
        ]
