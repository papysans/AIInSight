"""
工作流状态管理器
用于跟踪当前运行的工作流状态
"""
from typing import Optional, Dict, Any
from datetime import datetime
import asyncio

class WorkflowStatusManager:
    """管理工作流状态"""
    
    def __init__(self):
        self._status: Dict[str, Any] = {
            "running": False,
            "current_step": None,
            "progress": 0,
            "started_at": None,
            "topic": None,
            "current_source": None,  # 当前正在检索的来源
        }
        self._lock = asyncio.Lock()
    
    async def start_workflow(self, topic: str):
        """开始工作流"""
        async with self._lock:
            self._status = {
                "running": True,
                "current_step": "source_retriever",
                "progress": 0,
                "started_at": datetime.now().isoformat(),
                "topic": topic,
                "current_source": None,
            }
    
    async def update_step(self, step: str, progress: int = None, current_source: Any = "UNCHANGED"):
        """更新当前步骤"""
        async with self._lock:
            if step != self._status["current_step"] and current_source == "UNCHANGED":
                self._status["current_source"] = None

            self._status["current_step"] = step
            if progress is not None:
                self._status["progress"] = progress
            else:
                step_progress = {
                    "source_retriever": 10,
                    "reporter": 25,
                    "analyst": 40,
                    "debater": 60,
                    "writer": 80,
                    "image_generator": 95,
                }
                self._status["progress"] = step_progress.get(step, self._status["progress"])

            if current_source != "UNCHANGED":
                self._status["current_source"] = current_source
    
    async def finish_workflow(self):
        """完成工作流"""
        async with self._lock:
            self._status = {
                "running": False,
                "current_step": None,
                "progress": 100,
                "started_at": self._status.get("started_at"),
                "topic": self._status.get("topic"),
                "current_source": None,
            }
    
    async def get_status(self) -> Dict[str, Any]:
        """获取当前状态"""
        async with self._lock:
            return self._status.copy()
    
    async def reset(self):
        """重置状态"""
        async with self._lock:
            self._status = {
                "running": False,
                "current_step": None,
                "progress": 0,
                "started_at": None,
                "topic": None,
                "current_source": None,
            }


# 全局实例
workflow_status = WorkflowStatusManager()
