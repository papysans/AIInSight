...
- **本地开发/自托管模式**：`api + mcp + renderer + xhs-mcp` 四服务栈
- **远程 Gateway 模式（目标主路径）**：用户只连接一个远程 MCP Gateway，云端内部承载 `api + renderer + xhs-mcp`，宿主端负责 debate 编排
...
- 兼容路径仍支持直接执行 `analyze_topic(topic, depth="standard", image_count=0)`
- 新路径支持把分析拆成两段：
  1. `retrieve_and_report`（云端：证据检索 + reporter）
  2. 宿主端 debate（analyst/debater）
  3. `submit_analysis_result`（云端：writer + image + optional publish）
...
这是当前仓库默认支持的**开发/自托管**主链路：**`api + mcp + renderer + xhs-mcp` 4 容器一起启动**。
...
当 AIInSight 部署为云端服务时，推荐用户只接入一个远程 MCP Gateway，例如：
...
- `api` / `renderer` / `xhs-mcp` 都位于云端私网
- 单话题分析默认走：
  1. `retrieve_and_report`
  2. 宿主端 debate
  3. `submit_analysis_result`
- `analyze_topic` 全包式路径仍作为回退兼容保留

### 多账号隔离

远程 Gateway 模式下，每个 API key 对应一个独立 account：

- 独立 XHS 登录态（ShunL `account` 参数）
- 独立分析任务空间
- 独立用户设置与密钥配置
- 独立输出目录、二维码和卡片预览
